...
export default function BarberoDashboard() {
  ...
  const loadBarberData = useCallback(async () => {
    ...
    try {
      ...
      try {
        const barberosResponse = await fetch('http://localhost:8000/api/barberos/', {
          headers: {
      ...
      const appointmentsResponse = await fetch('http://localhost:8000/api/citas/', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
      ...
      try {
        const statsResponse = await fetch('http://localhost:8000/api/estadisticas/', {
          headers: {
            'Authorization': `Bearer ${token}`
          }
        })
